...
sub notify_close {
    ...
    !$_->is_ready && $_->fail('Server connection is no longer active', redis => 'disconnected') for values %{$self->{subscription_channel}};
    $self->{subscription_channel} = {};
    !$_->is_ready && $_->fail('Server connection is no longer active', redis => 'disconnected') for values %{$self->{subscription_pattern_channel}};
